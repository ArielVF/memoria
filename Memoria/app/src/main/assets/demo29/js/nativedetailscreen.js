...
var World = {
    ...
    loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiDataZone) {
        ...
        for (var currentPlaceNr = 0; currentPlaceNr < poiDataZone.length; currentPlaceNr++) {
                    var singlePoi = {
                        "id": poiDataZone[currentPlaceNr].id,
                        "latitude": poiDataZone[currentPlaceNr].latitude,
                        "longitude": poiDataZone[currentPlaceNr].longitude,
                        "altitude": AR.CONST.UNKNOWN_ALTITUDE,
                        "title": poiDataZone[currentPlaceNr].name,
                        "description": poiDataZone[currentPlaceNr].description,
                        "photo": poiDataZone[currentPlaceNr].photo
                    };
        ...
        World.updateStatusMessage(currentPlaceNr + ' Zonas añadidas');
    ...
    updateDistanceToUserValues: function updateDistanceToUserValuesFn() {
        ...
        //alert(World.userLocation.longitude);
            World.markerList[i].distanceToUser = World.markerList[i].markerObject.locations[0].distanceToUser();
            //alert(World.markerList[i].markerObject.locations[0].latitude);
        }
    },
    ...
    */
    ...
    onMarkerSelected: function onMarkerSelectedFn(marker) {
        ...
        */
        ...
        if(marker.poiData.photo.localeCompare("sin imagen") == 0){
            document.getElementById("poiPhoto").src = "https://cdn.shopify.com/s/files/1/0266/8534/1738/products/imagen_no_disponible_7e1d45b9-12d2-4e64-afc2-d9abdffa377a_1024x1024.png?v=1620153742"
        }else{
            document.getElementById("poiPhoto").src = marker.poiData.photo;
        }
    ...
    updateRangeValues: function updateRangeValuesFn() {
        ...
        var maxRangeValue = (maxRangeMeters > 999) ?
            ((maxRangeMeters / 100).toFixed(2) + " km") :
    ...
    showRange: function showRangeFn() {
        if (World.markerList.length > 0) {
            ...
            World.updateStatusMessage('Sin lugares disponibles', true);
        }
    },
    ...
    reloadPlaces: function reloadPlacesFn() {
        ...
        if (!World.isRequestingData) {
            if (World.userLocation) {
                ...
                World.updateStatusMessage('Localización de usuario desconocida.', true);
            }
        } else {
            World.updateStatusMessage('Solicitando lugares...', true);
        }
    },

    /* Request POI data. */
    requestDataFromServer: function requestDataFromServerFn(lat, lon) {
        World.loadPoisFromJsonData(poiDataZone);
    },
...
getData: function getData(zoneData) {
    poiDataZone = zoneData;
}
